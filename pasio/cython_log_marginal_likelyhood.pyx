...
cdef class LogMarginalLikelyhoodIntAlphaComputer(LogMarginalLikelyhoodComputer):
    ...
    cpdef all_suffixes_self_score(self, int stop):
        ...
        shifted_segment_count_vec = (self.int_alpha + self.cumsum[stop]) - self.cumsum[0:stop]

        segment_length_vec = self.split_candidates[stop] - self.split_candidates[:stop]

        add_vec = self.log_gamma_computer.compute_for_array(shifted_segment_count_vec, max_value=(self.int_alpha + self.cumsum[stop]))
